...
model2 = torchvision.models.detection.keypointrcnn_resnet50_fpn(pretrained=True)
model2.eval()
...
test = {id: read_image("smiling_front/" + id + "_08.jpg") for id in ids}
m1 = model2([torch.Tensor(test["001"]).permute(2, 0, 1)/255.0])
img = (test["001"])
...
thickness = 2
img2 = img.copy()
for x,y,z in m1[0]["keypoints"][0][:5]:
    img2 = cv2.circle(img2, (x,y), radius, color, thickness)

# Displaying the image
plt.imshow(img2)

# use resnet for each photo
photos_by_id = {id: dict({"neutral": get_embed(neutral_faces_detection[id]),
                         "smiling": get_embed(smile_faces_detection[id])}) for id in ids}
#
